# Generated by Powerlevel10k configuration wizard on 2026-02-22 at 01:38 +03.
...
() {
  ...
  typeset -g POWERLEVEL9K_PYENV_CONTENT_EXPANSION='(pyenv${P9K_CONTENT})'
  # Custom icon.
  # typeset -g POWERLEVEL9K_PYENV_VISUAL_IDENTIFIER_EXPANSION='⭐'
...
function prompt_my_py_status() {
  # جلب رقم الإصدار من pyenv
  local v=$(pyenv version-name 2>/dev/null)
  
  # إذا لم يكن هناك إصدار مخصص أو كان إصدار النظام، لا تعرض شيئاً
  [[ -z "$v" || "$v" == "system" ]] && return

  # التحقق من حالة البيئة الوهمية
  if [[ -n "$VIRTUAL_ENV" ]]; then
    # البيئة مفعلة: اللون أبيض (15)، النص (pyenv3.10.14)
    p10k segment -f 15 -t "(pyenv${v})"
  else
    # البيئة غير مفعلة: اللون أزرق (4)، النص (py3.10.14)
    # ملاحظة: إذا كان الأزرق (4) غامقاً جداً، يمكنك تغييره إلى (39) لأزرق فاتح
    p10k segment -f 4 -t "(py${v})"
  fi
}
